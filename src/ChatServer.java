...
public class ChatServer {
	...
	private static HashMap<String, PrintWriter> details = new HashMap<>();
			...
			try {
				...
				while (true) {
					...
					if (input.contains(">>")) {
						...
						for (HashMap.Entry<String, PrintWriter> entry : details.entrySet())
							if (entry.getKey().matches(word[0])) 
							{
								PrintWriter writer = entry.getValue();
								writer.println("MESSAGE " + name + ": " + word[1]);
							}
					} 
					else 
					{
